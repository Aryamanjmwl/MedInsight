...
type Props = RecordHeaderProps & { reportCount: number; biomarkerCount: number; onUpload: () => void };

export function RecordHeader({ latestReportDate, reportCount, biomarkerCount, refreshing, onRefresh, onUpload }: Props) {
  ...
  return (
    <View style={[styles.container, isDesktop && styles.desktopContainer, isCompact && styles.compact]}>
      <View style={styles.heading}>
        <AppText variant="metadata" color="textMuted" style={styles.date}>{formatMonthHeader(latestReportDate)}</AppText>
